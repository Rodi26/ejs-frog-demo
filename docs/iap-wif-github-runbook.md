...
| `jf rt ping` → **`Invalid IAP credentials: JWT signature is invalid`** alors que le `curl` IAP est OK | **Normal** si le load balancer exige un JWT **Google** dans `Authorization` : **`jf` envoie le jeton JFrog**, que IAP interprète comme JWT Google → signature invalide. Il faut un **autre chemin** (hostname interne sans IAP, proxy qui combine en-têtes, ou configuration plateforme). |
